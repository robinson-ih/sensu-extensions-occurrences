...
module Sensu
  module Extension
    class Alertonce < Filter
       ...
       @@extension_name = "flap_handle_alert_once"
      ...
      def description
        "Handle flapping events and filter events using event occurrences & sensu stashes to store teh previous notification to reduce noise."
      end
      ...
      def event_filtered?(event)
        ...
        elsif event[:occurrences] < occurrences && event[:action] == :flapping
          response = api_request('/stashes/' + @filters[name]['channel_name'] + '/' + event[:client][:name] + '/' + event[:check][:name], 'GET', event)
          if response.code != '200' # if Not notified Already
